...
class TrackAnime extends Command{
    ...
    async execute(message: Message, commandArgs: string[]){
        var channel: TextChannel = <TextChannel>message.channel;
        if(commandArgs[0]){
    ...
    sendTrackingEmbed(channel: TextChannel, message: Message){
        var description = '';
        ...
        this.client.animeManager?.animeChannels.forEach(async ch => {
            if(ch.channel_id == channel.id){
                ...
                if(trackings.length){
                    var animeDetails = await <any>this.client.animeManager?.getAnimeFromIDArray(trackings);
                    
                    animeDetails.forEach((detail: any) => {
                ...
                message.channel.send({
                    embeds: [{
                        title: '#'+ (channel as TextChannel).name +' tracking list',
                        color: 'BLUE',
    ...
    async addTrack(channel_id: string, anime_id: number){
        var trackings: number[] = [];
        var ok = false;
    ...
    async clearTrack(channel_id: string){
    ...
    async removeTrack(channel_id: string, anime_id: number){
        var ok = false;
        var new_trackings: number[] = [];
    ...
    isInTracking(channel_id: string, anime_id: number){
        var found = false;
