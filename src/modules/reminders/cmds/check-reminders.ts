...
import { Message, MessageEmbedOptions, MessageOptions, TextChannel } from 'discord.js';
...
class CheckReminder extends Command{
    ...
    async execute(message: Message, commandArgs: string[]){
        ...
        if(commandArgs[0]){
            ...
            if(reminders.length){
                ...
                const channel = <TextChannel>this.client.channels.cache.get(channel_id);
                var channel_tag = 'DM';

                if(String(channel.type) != 'DM'){
    ...
    shortText(text: string, length = 20) {
        if (text == null) {
            return "";
        }
